...
struct VS_OUTPUT
{
    ...
    float3 playerViewDir : TEXCOORD1;
    

    float4 lightViewPosition[2][2] : LIGHTVIEWS;
...
VS_OUTPUT main(VS_INPUT input)
{
   ...
   // }
    ...
    for (int j = 0; j < 2; j++)
    {
        for (int i = 0; i < 2; ++i)
        {
            output.lightViewPosition[j][i] = mul(float4(input.inPos, 1.0f), worldMatrix);
            output.lightViewPosition[j][i] = mul(output.lightViewPosition[j][i], lightViewMatrix[j][i]);
            output.lightViewPosition[j][i] = mul(output.lightViewPosition[j][i], lightProjectionMatrix[j][i]);
