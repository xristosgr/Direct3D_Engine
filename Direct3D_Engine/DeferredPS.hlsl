...
Texture2D ssaoTexture : TEXTURE : register(t10);
...
float3 shadowLights(PS_INPUT input, float3 albedo, float3 bumpNormal, float roughness, float metallic, float3 V, float3 F0, int index, float3 worldPos);
...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    float3 albedo = pow(albedoRoughTexture.Sample(SampleTypeWrap, input.inTexCoord).xyz, 2.2f).rgb;
    float roughness = albedoRoughTexture.Sample(SampleTypeWrap, input.inTexCoord).a;
    float3 normal = normalMetalTexture.Sample(SampleTypeWrap, input.inTexCoord).xyz;
    float metallic = normalMetalTexture.Sample(SampleTypeWrap, input.inTexCoord).a;
    float3 worldPos = positionTexture.Sample(SampleTypeWrap, input.inTexCoord).xyz;
    float ssao = ssaoTexture.Sample(SampleTypeWrap, input.inTexCoord).r;
    //float ao = albedoRoughTexture.Sample(SampleTypeWrap, input.inTexCoord).a;
   ...
   // float ao = normalTexture.Sample(SampleTypeWrap, input.inTexCoord).r;
    ...
    if (enableSunLight)
        Lo += sunLight(input, albedo.rgb, normal, roughness, metallic, V, F0);
    for (int i = 0; i < 2; i++)
    {
        ...
        if (distance < Radius1[i].x)
        {
            if (enabledLights[i].x == 1)
            {
                ...
                Lo += shadowLights(input, albedo.rgb, normal, roughness, metallic, V, F0, i, worldPos);
   ...
   // {
   //     float distance = length(dynamicLightPosition[i + active_lights].xyz - worldPos);
   //     if (distance < Radius2[i].x)
   //     {
   ...
   //
   //         
   //             Lo1 += shadowLights(input, albedo.rgb, normal, roughness, metallic, V, F0, i + active_lights, worldPos);
   //        
   //          
   //         }
   //     }
   ...
   // }
    ...
    float3 I = normalize(worldPos - cameraPos);
...
float3 shadowLights(PS_INPUT input, float3 albedo, float3 bumpNormal, float roughness, float metallic, float3 V, float3 F0, int index, float3 worldPos)
{
    ...
    float distance = length(dynamicLightPosition[index].xyz - worldPos) * dynamicLightAttenuationAndStrength[index].w;
