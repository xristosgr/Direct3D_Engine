...
float4 main(PS_INPUT input) : SV_TARGET
{
   ...
   // input.inTexCoord.x += waterTranslation;
    NR_LIGHTS = lightsNR;
    ...
    float3 V = normalize(float3(0, 0, 0)  -input.inWorldPos);
    ...
    if (enableSunLight)
        Lo += sunLight(input, albedo.rgb, bumpNormal, 1.0f, metallic, V, F0);
    for (int i = 0; i < NR_LIGHTS; i++)
    {
        if (enabledLights[i].x == 1)
        {
            Lo += shadowLights(input, albedo.rgb, bumpNormal, 1.0f, metallic, V, F0, i);
        }
    ...
    float3 refectionMap = ((environmentTexture.Sample(SampleTypeWrap, R)) );
...
float3 sunLight(PS_INPUT input, float3 albedo, float3 bumpNormal, float roughness, float metallic, float3 V, float3 F0)
{
   ...
   // return (kD * albedo / PI + specular) * radiance * NdotL;
    ...
    float3 L = normalize(float3(0, 0, 0) - input.inWorldPos.xyz).xyz;
    float3 H = normalize(V + L);
        
    float distance = length(sunLightPosition.xyz);
    float attenuation = 1.0f / (distance * distance);
    float3 radiance = sunLightColor;
    ...
    float NdotL = max(dot(bumpNormal, L), 0.0f);
    return (albedo / PI + specular) * radiance * NdotL;
}
