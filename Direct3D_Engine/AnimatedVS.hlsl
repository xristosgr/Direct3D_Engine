...
VS_OUTPUT main(VS_INPUT input)
{
    ...
    for (int j = 0; j < lightsSize; j++)
    {
        ...
        output.lightViewPosition[j] = mul(float4(posL, 1.0f), worldMatrix);
        output.lightViewPosition[j] = mul(output.lightViewPosition[j], transpose(lightViewMatrix[j]));
        output.lightViewPosition[j] = mul(output.lightViewPosition[j], transpose(lightProjectionMatrix[j]));
