...
cbuffer lightCull : register(b4)
{
    ...
    float4 cutOff[MAX_NO_OF_LIGHTS];
}
...
Texture2D bloomTexture : TEXTURE : register(t11);
Texture2D depthMapTextures[MAX_NO_OF_LIGHTS] : TEXTURE : register(t12);

SamplerState SampleTypeClamp : register(s0);
SamplerState SampleTypeWrap : register(s1);
SamplerState SampleTypeLinear: register(s2);
...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    NR_LIGHTS = lightsNR;
    float4 albedo = float4(pow(objTexture.Sample(SampleTypeWrap, input.inTexCoord).xyz, gamma), 1.0f);
    //if (albedo.a < 0.1)
    //{
    //    discard;
    //}
   ...
   // {
    ...
    float3 bloom = bloomTexture.Sample(SampleTypeWrap, input.inTexCoord);
